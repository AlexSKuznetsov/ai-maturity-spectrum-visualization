...
const DiagramFooterControls: React.FC<DiagramFooterControlsProps> = ({
  ...
  return (
    <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50/30 dark:bg-dark-warm/50 flex items-center justify-between z-10">
      <button
        onClick={onPrevious}
        disabled={!activeLevel || activeLevel.id === 1}
        className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-slate-200 dark:border-slate-700 bg-white dark:bg-dark-warm-secondary hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:text-slate-200 h-9 px-4 py-2 shadow-sm"
      >
